...
```{r}
library(dplyr)
train=HR_data[sample(nrow(HR_data), 10000), ]
model <- glm(left ~ .,family="binomial",data=HR_data)
fitted_value = predict(model,newdata=HR_data,type='response')
for (i in 1:length(fitted_value)){
  if (fitted_value[i]<=0.5){
  fitted_value[i]=0
}else{
  fitted_value[i]=1
}}
accuracy=table(fitted_value==HR_data$left)[2]/length(HR_data[,1])
```
...
set.seed(1234)
test_number=sample(seq_len(nrow(modified_data)),size=floor(0.25*15000))
...
table(real_left==test$left)/floor(0.25*15000)
```
```{r}
